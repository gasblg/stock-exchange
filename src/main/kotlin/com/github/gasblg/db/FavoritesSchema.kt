...
import com.github.gasblg.db.entity.Favorites
...
class FavoritesSchema {
    ...
    suspend fun create(ticker: String, lang: String): SharesItemResponse? = dbQuery {
        Favorites.insert {
            it[Favorites.ticker] = ticker
        }
        ...
        Join(
            Favorites, i,
            onColumn = Favorites.ticker,
            otherColumn = i[Items.ticker],
            ...
            (c2[Candles.ticker].isNull()) and (i[Items.ticker] eq ticker)
        }.map {
            SharesItemResponse(
    ...
    suspend fun delete(ticker: String) {
        dbQuery {
            Favorites.deleteWhere { Favorites.ticker.eq(ticker) }
        }
    }
    ...
    suspend fun getAll(
        ...
        return try {
            dbQuery {
                ...
                val total = Favorites.selectAll().count()
